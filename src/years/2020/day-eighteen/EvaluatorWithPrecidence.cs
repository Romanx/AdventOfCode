...
{
    public class EvaluatorWithPrecidence : IEvaluator
    {
        private readonly IEnumerator<Token<ArithmeticExpressionToken>> _tokens;

        // Constructor - just store the tokenizer
        public EvaluatorWithPrecidence(TokenList<ArithmeticExpressionToken> tokens)
        {
        ...
        Node ParseMultiply()
        {
            ...
            while (true)
            {
                ...
                // Work out the operator
                Func<long, long, long>? op = null;
                if (current.Kind == ArithmeticExpressionToken.Times)
                {
                    op = (a, b) => a * b;
                }
